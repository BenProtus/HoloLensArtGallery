...
{
    /// <summary>
    /// The TapToPlaceOnWall class is a basic way to enable users to move objects 
    ...
    /// TapToPlaceOnWall also adds a WorldAnchor component to enable persistence.
    /// </summary>

    public class TapToPlaceOnWall : MonoBehaviour, IInputClickHandler
    {
        ...
        [Tooltip("This keeps track of which painting is currently being placed for the Iterate class to use.")]
        public int number;
        ...
        protected virtual void Update()
        {
            ...
            if (IsBeingPlaced)
            {
                ...
                if (Physics.Raycast(headPosition, gazeDirection, out hitInfo, 30.0f, spatialMappingManager.LayerMask))
                {
                    ...
                    {
                        toQuat = Quaternion.LookRotation(-hitInfo.normal, Vector3.up);
                    }
                    else
                    {
                        toQuat.x = 0f;
                ...
                else
                {
                    toQuat.x = 0f;
                    toQuat.z = 0f;
                    gameObject.transform.position = headPosition + gazeDirection * (GazeManager.Instance.MaxGazeCollisionDistance - 5);
                    gameObject.transform.rotation = toQuat;
                }
            }
            //If the user is not in placing mode, open the frame menu
            else
            {
